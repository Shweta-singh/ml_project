...
def train(opt):
    ...
    # resuming train_AREL capability
    if opt.resume_from is not None:
        print('resuming AREL training...\n')
        optim_path = os.path.join(opt.resume_from, "optimizer.pth")
        disc_optim_path = os.path.join(opt.resume_from, "disc-optimizer.pth")
        disc_model_path = os.path.join(opt.resume_from, "disc-model.pth")
    else:
        optim_path, disc_optim_path, disc_model_path = None, None, None
    ...
    if opt.resume_from is not None:
        disc_optim_path = os.path.join(opt.resume_from, "optimizer.pth")
        disc_model_path = os.path.join(opt.resume_from, "model.pth")
        disc.load_state_dict(torch.load(disc_model_path))
        disc.load_state_dict(torch.load(disc_optim_path))
    ...
    for epoch in range(logger.epoch_start, opt.max_epochs):
	    ...
	    #print('#############################################', gen_score.shape, normed_seq_log_probs.shape)
            if flag.flag == "Disc":
		...
		# print('$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$', gen_score.shape, normed_seq_log_probs.view(1,-1).shape)
                rewards = Variable(gen_score.data - 0 * normed_seq_log_probs.view(1,-1).data)
                ...
                logging.info(
                    "average reward: {} average IRL score: {}".format(avg_score.data, avg_pos_score.data))
            ...
            if flag.flag == "Disc":
                ...
                # print("rl_loss / tf_loss = ", loss.data / tf_loss.data)
            ...
            if logger.iteration % opt.save_checkpoint_every == 0:
                if opt.always is None:
                    ...
                    torch.save(disc_optimizer.state_dict(), os.path.join(logger.log_dir, 'disc-optimizer.pth'))
