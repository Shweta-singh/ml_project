...
class VISTDataset(Dataset):
    def __init__(self, opt):
        ...
        # self.story_line['test'] = {A: N for (A, N) in [x for x in self.story_line['test'].items()][:10]}
        # self.story_line['image2caption']['test'] = {A: N for (A, N) in [x for x in self.story_line['image2caption']['test'].items()][:10]}
        #
        # self.story_line['image2caption_original']['test'] = {A: N for (A, N) in [x for x in self.story_line['image2caption_original']['test'].items()][:10]}
        # self.story_line['album2stories']['test'] = {A: N for (A, N) in [x for x in self.story_line['album2stories']['test'].items()][:10]}

        # # self.story_line['test'] = {'46759': self.story_line['test']['46759']}
        # # self.story_line['image2caption']['test'] = {'46759': self.story_line['image2caption']['test']['46759']}
        # #
        # # self.story_line['image2caption_original']['test'] = {'46759': self.story_line['image2caption_original']['test']['46759']}
        # # self.story_line['album2stories']['test'] = {'46759': self.story_line['album2stories']['test']['46759']}
        # #
        #
        ...
        # test_id_list = []
        # for key,values in self.story_line['test'].iteritems():
        #     for idf in values['flickr_id']:
        #         test_id_list.append(str(int(idf)))
        # with open('listfile.txt', 'w') as filehandle:
        #     for listitem in test_id_list:
        #         filehandle.write('%s\n' % listitem)
    ...
    def __getitem__(self, index):
        if self.task == 'story_telling':
            ...
            for i in xrange(story['length']):
                ...
                # if self.mode == 'test':
                    # print(len(story['flickr_id'][i]))
                    # test_id_list.append(str(int(story['flickr_id'][i])))
            # print(test_id_list)
